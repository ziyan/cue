...
The management interface has a sidebar instead of a row of tabs, and the settings are several pages instead of one. On a phone the tabs were 167 pixels wider than the screen, so Device and Upgrade sat off the right-hand edge with nothing to say they were there — including the page that tells you a newer version exists. The Device page was forty-two settings on one page, seven screens tall on a phone, with three of them hidden inside collapsed boxes labelled "Less often needed" and "Difficult hardware" — labels about how often you might want a setting rather than what it was. Finding one meant opening every box. It is now Device, Display, Browser, Health, Access and Logs, four to eleven settings each and nothing collapsed. Light and dark is a choice rather than whatever the machine you are looking from happens to be set to, in three states, remembered in that browser. The Logs page reads the log when you open it rather than after you press a button, and no page scrolls sideways any more. The pointer can be seen on the Screen page. A screen started without a cursor sends no cursor image, and the picture was left with no pointer at all, so it could not be aimed. There is a dot where the pointer is. Fullscreen works on a phone. Safari on an iPhone has no fullscreen for anything but a video, so the button did nothing there; when the browser refuses, the page fills the window itself, with an X in the corner. The picture on the Overview page goes to the Screen page when pressed. The language control is a globe rather than a translate mark, the light and dark control is a sun or a moon rather than the icon for whichever of the three settings was picked, and on a phone the control that opens the navigation is the mark rather than an anonymous hamburger. A settings page draws as soon as its settings arrive. It used to wait for the status as well, which only the Device page reads, and showed the word "Loading" until both had answered. The interface is compressed and can be cached. A first load was 619,949 bytes and is now 190,270, and a revisit is answered with 304 and an empty body rather than the whole interface again -- the files are embedded in the executable, so they had no modification time, no ETag, and nothing the browser could ask about. Asking for the headers of a page or a file with HEAD is answered rather than refused. The X on the on-screen menu answers the first press. It was small, it gave no sign of having been pressed while the daemon put the tab back, and it needed the press and the release to land on the same spot -- which a pointer driven over VNC does not reliably do. (#5)
## [0.3.0] - 2026-08-28
